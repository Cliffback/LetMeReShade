...
const manageGameReShade = callable<
  [string, string, string, string, string],
  ReShadeResponse
>('manage_game_reshade');
const checkReShadePath = callable<[], PathCheckResponse>('check_reshade_path');
const listInstalledGames = callable<[], GameListResponse>(
  'list_installed_games',
);
const findGameExecutablePath = callable<[string], ExecutableDetectionResponse>(
  'find_game_executable_path',
);
...
const SteamGamesSection = () => {
  ...
  const [executableDetection, setExecutableDetection] =
    useState<ExecutableDetectionResponse | null>(null);
    ...
    const checkExecutableDetection = async () => {
      ...
      try {
        ...
        if (detection.status === 'success') {
          if (
            detection.recommended_method === 'steam_logs' &&
            ...
            detection.enhanced_detection_result?.status === 'success'
          ) {
            setSelectedExecutablePath(
  ...
  const getLinuxDetectionInfo = () => {
    ...
    if (
      enhancedResult &&
      (enhancedResult.is_linux_game ||
        enhancedResult.status === 'linux_game_detected')
    ) {
      return {
        is_linux_game:
          enhancedResult.is_linux_game ||
    ...
    if (
      steamResult &&
      (steamResult.is_linux_game ||
        steamResult.status === 'linux_game_detected')
    ) {
      return {
        is_linux_game:
          steamResult.is_linux_game ||
      ...
      const LinuxGameModalContent = () => (
        <div style={{ textAlign: 'left' }}>
          <p style={{ marginBottom: '16px' }}>
            This appears to be a Linux version of{' '}
            <strong>{selectedGame.name}</strong>. ReShade only works with
          ...
          <div style={{ marginBottom: '16px', paddingLeft: '8px' }}>
            ...
            <div style={{ marginBottom: '4px' }}>
              • Check "Force the use of a specific Steam Play compatibility
              tool"
            </div>
            <div style={{ marginBottom: '4px' }}>
              • Select "Proton Experimental" or latest Proton version
            </div>
            <div style={{ marginBottom: '4px' }}>
              • Reinstall the game to download the Windows version
            </div>
          </div>
          ...
          onOK={async () => {
            ...
            if (response.status === 'success') {
              ...
              if (selectedDll.value === 'auto' && response.output) {
                const launchOptionsMatch = response.output?.match(
                ...
                if (launchOptionsMatch) {
                  ...
                  setResult(
                    `Successfully patched ${selectedGame.name} with ${dllValue.toUpperCase()}.\nPress HOME key in-game to open ReShade overlay.`,
                  );
                }
              } else {
              ...
              setResult(
                `Failed to patch: ${response.message || 'Unknown error'}`,
              );
            }
          }}
        />,
        ...
        <ConfirmModal
          ...
          onOK={async () => {
            ...
            if (response.status === 'success') {
              ...
              setResult(
                `Successfully removed ReShade from ${selectedGame.name}`,
              );
            } else {
              setResult(
                `Failed to unpatch: ${response.message || 'Unknown error'}`,
              );
            }
          }}
        />,
          ...
          <PanelSectionRow>
            <div
              style={{
                ...
                marginTop: '8px',
                backgroundColor:
                  linuxInfo.confidence === 'high' ? '#ff6b6b' : '#ffa726',
                borderRadius: '4px',
                color: 'white',
              }}
            >
              ...
              <div style={{ fontSize: '0.9em', marginBottom: '8px' }}>
                This appears to be a Linux version. ReShade requires Windows
                version through Proton.
              </div>
              <div style={{ fontSize: '0.85em' }}>
                <strong>Fix:</strong> Properties → Compatibility → Force Proton
                → Reinstall game
              </div>
              {linuxInfo.reasons && linuxInfo.reasons.length > 0 && (
                <div
                  style={{ fontSize: '0.8em', marginTop: '8px', opacity: 0.9 }}
                >
                  <strong>Detected:</strong>{' '}
                  {linuxInfo.reasons.slice(0, 2).join(', ')}
                </div>
              )}
              {linuxInfo.scan_summary && (
                <div
                  style={{ fontSize: '0.8em', marginTop: '4px', opacity: 0.9 }}
                >
                  Found: {linuxInfo.scan_summary.so_files} .so files,{' '}
                  {linuxInfo.scan_summary.sh_files} .sh files
                </div>
              )}
            </div>
          </PanelSectionRow>
        ...
        {enhancedResult?.scan_summary && (
          <PanelSectionRow>
            ...
            >
              <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>
                📊 Scan Results:{' '}
                {enhancedResult.scan_summary.total_files_scanned} files analyzed
              </div>
              <div>
                Windows: {enhancedResult.scan_summary.main_windows_executables}{' '}
                executables • Linux: {enhancedResult.scan_summary.so_files} .so
                files, {enhancedResult.scan_summary.sh_files} .sh files
              </div>
            </div>
          </PanelSectionRow>
        )}
  ...
  const renderExecutableSelection = () => {
    ...
    if (
      ...
      enhancedResult.all_executables.forEach((exe, index) => {
        ...
        const steamLogsFilename =
          steamLogsResult?.filename ||
          steamLogsResult?.executable_path?.split('/').pop();
        const isDuplicate =
          steamLogsFilename && steamLogsFilename === exe.filename;

        if (!isDuplicate) {
          const isRecommended =
            recommendedMethod === 'enhanced_detection' &&
            exe.path === enhancedResult.executable_path;
          executableOptions.push({
            path: exe.path,
        ...
        <>
          ...
          {selectedGame && checkingExecutable && (
            <PanelSectionRow>
              <div style={{ fontSize: '0.9em', opacity: 0.7 }}>
                🔍 Analyzing game directory (Windows/Linux detection +
                executable analysis)...
              </div>
            </PanelSectionRow>
          )}
          ...
          {selectedGame && (
            <>
              ...
              <PanelSectionRow>
                <ButtonItem layout="below" onClick={handleUnpatch}>
                  🗑️ Remove ReShade
                </ButtonItem>
              </PanelSectionRow>
            </>
